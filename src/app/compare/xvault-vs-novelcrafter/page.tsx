...
const rows: { feature: string; xvault: React.ReactNode; opponent: React.ReactNode; note?: string }[] = [
  ...
  { feature: "Predictable monthly cost",   xvault: <Yes />,     opponent: <No />,      note: "NovelCrafter's effective cost depends on how much you write — API costs scale with usage." },
...
NovelCrafter's bring-your-own-key model is a deliberate architectural choice. It gives writers flexibility — you can connect any supported model, switch between them, and take advantage of the latest capabilities without waiting for NovelCrafter to update their infrastructure.

The problem is what this requires of the writer. To use NovelCrafter fully, you need to:

- Create an account with OpenAI, Anthropic, or Google
- Understand API billing and set spending limits
- Manage API keys securely
- Monitor usage to avoid unexpected charges

For technically comfortable writers, this is a minor inconvenience. For the majority of novelists — who want a writing tool, not a software infrastructure project — it is a meaningful barrier, and the unpredictability of API costs makes budgeting difficult.

The effective monthly cost for a writer using NovelCrafter actively lands between $24 and $44 per month once API costs are included. The subscription starts at $4 per month, but that number is somewhat misleading without the context of what you will pay your AI provider on top of it.

Xvault's pricing is $11.99 per month or $119 per year. No API keys. No separate billing account. No usage-based surprises.

## The Codex vs. Automatic Extraction

NovelCrafter's Codex is a well-designed worldbuilding system. You record your characters, locations, lore, and factions, and the AI references that Codex when generating suggestions. For writers who want structured control over their world data, it is genuinely good.

The limitation is that the Codex is built manually. You record what you have established. You update it when things change. You decide what goes in and what does not. If you forget to add something, it is not in the Codex. If the AI is asked about something not in the Codex, it does not know it exists.
...
This is not better in every situation. If you want to predefine your world before writing — to build the Codex in advance and have the AI work from that structure — NovelCrafter's approach has advantages. If you want the system to learn your world as you write it, Xvault's automatic extraction is the closer fit.

## Where NovelCrafter Is Genuinely Better

NovelCrafter's 157,000-user community is a real asset. There is an active forum, shared resources, and collective knowledge about how to use the tool effectively. When you encounter a problem, there is a community that has likely encountered it before.

The flexibility in AI model selection is also a genuine differentiator for power users. If you have strong opinions about which model produces the best fiction prose — or if you want to experiment with local models for privacy or cost reasons — NovelCrafter accommodates that in a way Xvault currently does not.

## Who Should Use What

**Use NovelCrafter if:** You are technically comfortable managing API keys and separate billing, you want flexibility in which AI models you use, and the structured Codex system appeals to your worldbuilding workflow.

**Use Xvault if:** You want a single predictable monthly cost, you want your world board and story bible to build themselves from your writing, you want voice matching that keeps AI suggestions sounding like you, or you simply want a tool that requires no API infrastructure to set up.
